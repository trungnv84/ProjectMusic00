...
> **AI:** Đây là trang **bắt buộc** ở Bước 3. Kho hướng dẫn **cách nghĩ và kiểm**, không cấp sẵn chuỗi nốt để copy. Mọi ví dụ pitch chỉ minh họa kỹ thuật — **cấm** tái sử dụng làm skeleton cho cả bài. Bắt buộc kết hợp [catchiness](catchiness.md).
...
Lỗi thứ hai: phrase **rời rạc** không motif thống nhất, hoặc lặp cell mà cell gốc **không memorable** → nghe nhàm / không catchy. Xem [catchiness](catchiness.md).
...
- **Hook Chorus phải là câu đáng nhớ trước**, rồi mới lặp; lặp skeleton nhạt ≠ hook. Phải invent `hook_melody_cell` ngắn (xem [catchiness](catchiness.md)) **trước** khi viết full Chorus.
- Verse 2 / Final: biến thể có chủ đích (rhythm, opening, cadence, sequence…) — không chỉ "cùng nốt cao hơn".
- Sau khi viết, tự hỏi:
  - *Nếu bỏ lời, các câu Verse còn phân biệt được không?* Nếu không → FAIL và viết lại.
  - *Hook cell có muốn ngân nga sau 1–2 lần không?* Nếu không → FAIL và invent lại cell.
- Cấm phrase rời rạc không chia sẻ DNA motif trong cùng section (`melodic_coherence`).

## Hints — quy trình invent (nội bộ, không file riêng)

1. **Nghe lời nói:** đọc câu hát theo nhịp nói; đánh dấu từ mang nghĩa.
2. **Rhythm trước pitch (hoặc đồng thời):** đặt chỗ nhấn / ngân / thở trên bar (xem lyric-melody-fit) — mỗi câu có thể khác nhịp một chút.
3. **Một câu hook trước:** thử vài contour + rhythm cell ngắn (2–5 nốt / 2–4 bar); chọn câu muốn ngân nga; **khóa `hook_melody_cell` vào notes** (bắt buộc). Tham khảo đặc trưng catchy: clear contour, limited pitch set, rhythmic identity.
4. **Verse = kể chuyện:** câu hỏi → câu trả lời (A/B), đổi hướng hoặc nhịp ở câu sau; đừng clone A; vẫn giữ coherence motif.
...
7. **Hát thầm toàn bài** (speak/sing-test) trước khi xuất XML; chỗ đều đều / rời rạc / không dính → viết lại đoạn đó.
...
1. Invent hook cell + 1–2 motif section (ghi `motifs_declared` / `hook_melody_cell` + evidence catchiness trong **composition notes**).
2. Viết từng phrase có mục tiêu riêng; so với anti-patterns + catchiness.
3. Xuất pretty-print XML + notes + quality gate trong **cùng Bước 3**.
4. Gate FAIL → đổi ý nhạc (kể cả hook cell nếu cần), sinh lại — không patch nốt lẻ.
