(() => {
  ...
  const USER_CONTENT_SELECTORS = [
    ...
    '[class*="query-text"]', '[class*="user-content"]'
  ];
  ...
  const QUOTA_MARKERS = [
    ...
    /slow down/i, /temporarily unable/i, /please try again/i
  ];
  ...
  function detectUiState() {
    ...
    try {
      ...
      if (!state.loading) {
        const spinners = document.querySelectorAll('[class*="spin"], [class*="loader"], [role="progressbar"], svg[class*="spin"], [class*="progress-indicator"]');
  ...
  async function waitForAnswer(prompt, alreadySent, timeoutMs) {
    ...
    while (Date.now() < deadline) {
      ...
      if (ui.quotaError) return { ok: false, error: `Gemini: hết lượt/quota (banner lỗi). ${ui.errorBannerText ? '→ ' + ui.errorBannerText : ''}` };
      const ctx = getConversationContext(prompt);
    ...
    if (uiEnd.quotaError) return { ok: false, error: `Gemini: hết lượt/quota khi chờ kết quả. ${uiEnd.errorBannerText ? '→ ' + uiEnd.errorBannerText : ''}` };
    if (uiEnd.loading) return { ok: false, error: 'Gemini: hết thời gian chờ (AI vẫn đang xử lý, tab có thể treo).' };
    return { ok: false, error: `Gemini: hết thời gian chờ. Hãy refresh tab hoặc kiểm tra giao diện. (Tìm thấy ${ctxEnd.assistantCount} assistant row; pair=${ctxEnd.matchedUserIdx>=0?'Y':'N'})` };
  }

  async function run(prompt, timeoutMs) {
    const uiPre = detectUiState();
    if (uiPre.quotaError) return { ok: false, error: `Gemini: Hết lượt/quota (trước khi gửi). ${uiPre.errorBannerText ? '→ ' + uiPre.errorBannerText : ''}` };
  ...
  function peekAllMessages() {
    try {
      const arr = getAssistantAnswers();
      return { ok: true, provider: 'gemini', candidates: arr.map((t, i) => ({ index: i, text: t, length: t.length })).filter(c => c.length > 20) };
