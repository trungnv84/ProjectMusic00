(() => {
  ...
  const ASSISTANT_ROW_SELECTORS = [
    ...
    '[class*="turn-container"][class*="assistant" i]',
    'div[class*="message" i][class*="assistant" i]',
    '[class*="message" i][data-is-streaming]'
  ];
  ...
  const QUOTA_MARKERS = [
    ...
    /something went wrong/i, /we're experiencing high demand/i
  ];
  ...
  function detectUiState() {
    ...
    try {
      ...
      if (!state.loading) {
        const streaming = document.querySelectorAll('[data-is-streaming="true"], [class*="streaming"]');
        for (const el of streaming) if (!isElementGone(el)) { state.loading = true; break; }
      }
      if (!state.loading) {
        const spinners = document.querySelectorAll('[class*="spin"], [class*="loader"], [role="progressbar"], svg[class*="spin"], [class*="cursor-wait"]');
    ...
    try {
      const bannerSelectors = [
        ...
        '[data-testid*="error"]', '[class*="Dialog"] [class*="content"]'
      ];
  ...
  async function waitForAnswer(prompt, alreadySent, timeoutMs) {
    ...
    while (Date.now() < deadline) {
      ...
      if (ui.quotaError) return { ok: false, error: `ChatGPT: hết lượt/quota (banner lỗi). ${ui.errorBannerText ? '→ ' + ui.errorBannerText : ''}` };
      const ctx = getConversationContext(prompt);
    ...
    if (uiEnd.quotaError) return { ok: false, error: `ChatGPT: hết lượt/quota khi chờ kết quả. ${uiEnd.errorBannerText ? '→ ' + uiEnd.errorBannerText : ''}` };
    if (uiEnd.loading) return { ok: false, error: 'ChatGPT: hết thời gian chờ (AI vẫn đang xử lý, tab có thể treo).' };
    return { ok: false, error: `ChatGPT: hết thời gian chờ. Hãy refresh tab hoặc kiểm tra giao diện. (Tìm thấy ${ctxEnd.assistantCount} assistant row; pair=${ctxEnd.matchedUserIdx>=0?'Y':'N'})` };
  }

  async function run(prompt, timeoutMs) {
    const uiPre = detectUiState();
    if (uiPre.quotaError) return { ok: false, error: `ChatGPT: Hết lượt/quota (trước khi gửi). ${uiPre.errorBannerText ? '→ ' + uiPre.errorBannerText : ''}` };
  ...
  function peekAllMessages() {
    try {
      const arr = getAssistantAnswers();
      return { ok: true, provider: 'chatgpt', candidates: arr.map((t, i) => ({ index: i, text: t, length: t.length })).filter(c => c.length > 20) };
