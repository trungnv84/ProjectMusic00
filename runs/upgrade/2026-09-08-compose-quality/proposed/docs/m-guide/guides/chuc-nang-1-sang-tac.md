...
| 3 | `03a-composition-plan.md` → `03-song.musicxml` + notes (+ quality gate) | Lead sheet: lời, giai điệu, hòa âm |
| 4 | `04-arranged.musicxml` + notes | Phối khí nhiều part |

Mỗi bước **dừng để bạn sửa**. Không bắt buộc chạy cả 4 trong một lần. Bước 3 có thể dừng sau **3a** (duyệt plan) trước khi viết MusicXML.
...
Sau Bước 3: dừng sau 03a-composition-plan nếu tôi muốn duyệt plan; rồi lead sheet + music_quality_gate PASS (không full arrangement).
...
```text
Đọc https://github.com/trungnv84/ProjectMusic00/blob/master/docs/m-guide/for-ai.md
Làm Bước 3 theo pipeline/step-03-compose.md (pha 3a → 3b → 3c).
Compose-prompt: runs/compose/<run-id>/02-compose-prompt.md
Yêu cầu bài hát: "..."
REFERENCE_STYLE: STYLE.VN.VPOP-BALLAD

Fetch mọi DOC_REFS trong compose-prompt + composition-planning, melody anti-patterns, quality-gate, lyric-melody-fit + musicxml + song-request-schema.
Xuất:
- runs/compose/<run-id>/03a-composition-plan.md  (duyệt trước nếu tôi yêu cầu)
- runs/compose/<run-id>/03-song.musicxml  (lead sheet pretty-print)
- runs/compose/<run-id>/03-composition-notes.md  (lyrics_by_section + prosody_audit + music_quality_gate)
Cấm: full band; sao chép hook/lời bản quyền; bỏ qua thanh điệu / lyric–melody fit; patch nốt sau gate FAIL.
```
...
- [ ] Bước 3 = `03a` plan rồi lead sheet; Bước 4 mới full parts
- [ ] `03-composition-notes.md` có `lyrics_by_section` + `music_quality_gate: PASS`
- [ ] Tiếng Việt: tone-melody (transitions) + lyric-melody-fit + speak-test
