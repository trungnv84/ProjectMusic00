...
Phải ghi `music_quality_gate: PASS` hoặc `FAIL`. **PASS** chỉ khi mọi ngưỡng dưới đạt và không vi phạm [anti-patterns](anti-patterns.md) / [lyric-melody-fit](../lyrics/lyric-melody-fit.md) / [melody-invention](melody-invention.md) / [catchiness](catchiness.md) / [piano-reduction](../harmony/piano-reduction.md).
...
| `REQUIRE_CATCHY_HOOK` | Hook cell ngắn / rõ contour / có rhythmic identity; notes ghi evidence catchiness (contour type, limited pitch set?, rest/syncop?); cell muốn ngân nga sau 1–2 lần (tự đánh giá) — xem [catchiness](catchiness.md) |
...
| `REQUIRE_MELODIC_COHERENCE` | Trong mỗi section chính, phrases chia sẻ DNA motif (không rời rạc hoàn toàn) |
...
**Thiếu `hook_melody_cell` mô tả cụ thể hoặc `REQUIRE_CATCHY_HOOK: fail` → gate FAIL.**

## Hints

- Nghe đều đều / rời rạc / không muốn ngân nga trên Flat → ghi fail; không biện minh “đủ section”.
...
2. Điền `piano_texture` (bắt buộc) + `hook_melody_cell` + `motifs_declared` + evidence catchiness rồi bảng điểm + ngưỡng — gồm `REQUIRE_CATCHY_HOOK` và `REQUIRE_PIANO_TEXTURE`.
