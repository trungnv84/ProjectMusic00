...
2. **Tự sáng tác** lời + giai điệu + hòa âm theo [melody-invention](../knowledge/melody/melody-invention.md) + **[catchiness](../knowledge/melody/catchiness.md)** — **cấm** điền lời vào một skeleton pitch/rhythm cố định rồi lặp; **bắt buộc invent `hook_melody_cell` ngắn đáng nhớ trước** khi viết full Chorus.
3. Trong đầu (hoặc nháp riêng, **không** bắt buộc artifact): chốt hook cell catchy (clear contour + rhythmic identity), motif khác nhau theo section, map prosody; rồi viết thẳng lead sheet. Kiểm coherence (không phrase rời rạc).
...
5. Viết `03-composition-notes.md`: `lyrics_by_section`, `motifs_declared`, **`hook_melody_cell` (mô tả pitch+rhythm cell + evidence catchiness)**, `prosody_audit`, `music_quality_gate` (gồm **REQUIRE_CATCHY_HOOK** + **REQUIRE_PIANO_TEXTURE**), **`piano_texture`** (bắt buộc — thiếu = gate FAIL).
6. Tự chấm gate gồm **`REQUIRE_CATCHY_HOOK`** và **`REQUIRE_PIANO_TEXTURE`**. **FAIL** (kể cả hook không memorable / phrase rời rạc / pad piano / thiếu `piano_texture`) → viết lại lead sheet **và** piano texture, **không** patch vài nốt, **không** “để Bước 4 sửa”. Chỉ coi xong khi PASS (hoặc user override tường minh).
...
STATUS.md  # step3: done chỉ khi gate PASS (gồm REQUIRE_CATCHY_HOOK + REQUIRE_PIANO_TEXTURE) hoặc user override
```
