...
## Phiên bản 2.2 — Tổng quát & Mở rộng (đồng bộ với Tài liệu 1 — Prompt Builder v3.0)
...
- **❌ MUSICXML ANTI-PATTERN #1:** Khai báo nhạc cụ trong `<part-list>` (ví dụ: Strings, Brass, Percussion) nhưng KHÔNG viết element `<part id="...">` thực tế tương ứng với mỗi nhạc cụ đó. Nếu không đủ token/không gian để viết hết cho N part, thì GIẢM số part trong part-list xuống còn số part thực tế có thể viết đầy đủ. Quy tắc: `số score-part TRONG part-list` PHẢI BẰNG `số element <part>` thực tế.
- **❌ MUSICXML ANTI-PATTERN #2:** Bỏ trống các measure trung gian trong một part (ví dụ: viết measure 1–5 rồi nhảy thẳng measure 56). Mọi `<part>` PHẢI chứa `<measure>` với `number` liên tục từ `1` đến `MAX_MEASURE` của bài (giống hệt part giọng hát). Không được thay element XML thực bằng comment dạng `<!-- measures 6–56 continue... -->`.
- **❌ MUSICXML ANTI-PATTERN #3:** Dùng XML comment kiểu `<!-- Full expansion omitted for response length -->`, `<!-- Placeholder continuation to keep schema valid -->`, hay bất kỳ comment nào "xin phép tắt" về việc cấu trúc MusicXML chưa đầy đủ. Comment dạng này KHÔNG biện minh cho schema invalid — nếu schema không hợp lệ thì file không được xuất.
- **❌ MUSICXML ANTI-PATTERN #4:** Thêm XSD schema declaration (`xmlns:xsi`, `xsi:noNamespaceSchemaLocation`) vào thẻ mở `<score-partwise>`. Chỉ dùng DTD validation duy nhất qua `<!DOCTYPE score-partwise PUBLIC "-//Recordare//DTD MusicXML 4.0 Partwise//EN" ...>` như ví dụ 11.1. DTD là chuẩn mặc định cho MusicXML 4.0.
- **❌ MUSICXML ANTI-PATTERN #5:** Viết `<midi-program>0</midi-program>` trong bất kỳ `<midi-instrument>` nào. Theo schema MusicXML 4.0, `midi-program` là kiểu `xs:positiveInteger` — **chỉ chấp nhận giá trị từ 1 đến 128** (không có 0). Đây là lỗi Fatal phổ biến: "Content of element midi-program does not match its type definition: 0 is not valid according to xs:positiveInteger". Lưu ý quan trọng: **MIDI chuẩn 0-based (program 0–127) phải chuyển sang MusicXML 1-based (1–128)** bằng cách +1 mọi giá trị. Ngay cả kênh trống percussion (channel 10) không dùng program thực tế, vẫn phải đặt một số dương hợp lệ (vd: 1), không được 0.
- **❌ MUSICXML ANTI-PATTERN #6:** Viết `<duration>0</duration>` bên trong một `<note>` (note/rest/chord). Lỗi này gây Fatal error tương tự và phá vỡ phép kiểm tra tổng trường độ measure. Mọi `<duration>` PHẢI là số nguyễn DƯƠNG (> 0). Nếu một từ có 2 âm tiết (syllabic begin + end) và tổng duration cần phân bổ không khớp, hãy **chia lại duration đều giữa 2 note** (vd: thay vì 4 + 0 → 2 + 2; hoặc 3 + 1) — không bao giờ đặt note cuối có duration = 0 để "cho qua". Nhắc lại: tổng duration mọi note trên cùng `<voice>` trong measure PHẢI BẰNG `divisions × beats × (4 / beat-type)`.
...
*(CẬP NHẬT — thêm các mục 11.4 → 11.7 kèm ví dụ ĐÚNG/SAI để ngăn chặn lỗi phổ biến DTD ordering, part-list mismatch, measure gap, placeholder-comment pattern, dual-schema conflict)*

### 11.1 Cấu trúc tối thiểu bắt buộc (KHÔNG DÙNG `...` PLACEHOLDER)
```xml
...
<score-partwise version="4.0">
  <work>
    <work-title>TÊN BÀI HÁT (không để ...)</work-title>
  </work>
  <identification>
    <creator type="composer">Tên người sáng tác / AI Music Composer</creator>
    <creator type="lyricist">Tên người viết lời</creator>
    <rights>Thông tin bản quyền / Original composition – no copyrighted material used</rights>
    <encoding>
      <software>Tên phần mềm / AI MusicXML Generator</software>
...
> ⚠️ **QUAN TRỌNG (chống lỗi #1 cũ):** THẺ MỞ `<score-partwise>` CHỈ ĐƯỢC VIẾT NHƯ VÍ DỤ TRÊN — **TUYỆT ĐỐI KHÔNG** thêm `xmlns:xsi="..."` hay `xsi:noNamespaceSchemaLocation="..."`. XSD declaration xung đột với DTD và gây "Fatal error: Loaded schema file is invalid". Chỉ dùng DTD một mình.
...
| `include_instruments = true` | `<part-list>` đầy đủ nhạc cụ, `<score-instrument>` + `<midi-instrument>` — **VÀ** phải có element `<part id="...">` thực tế cho **mỗi** score-part đã khai báo |
...
| `include_metadata = true` | `<work-title>`, `<creator>`, `<identification>` (với `<rights>` TRƯỚC `<encoding>` — xem 11.4) |
...
### 11.4 🚦 QUY TẮC SẮP XẾP ELEMENT THEO DTD — STRICT ORDERING (BẮT BUỘC)

MusicXML 4.0 DTD là **sequence-based** (hoàn toàn phụ thuộc vào TRÌNH TỪ element con). Nếu đặt element sai vị trí, trình xác thực sẽ báo:
> **Element `X` is not defined in this scope**

Hai cấu trúc dễ bị sai nhất (đã gây lỗi trong quá khứ) là `<identification>` và đa-staff `<attributes>`. Luôn tuân theo trình tự dưới đây.

#### 11.4.1 Trình tự BẮT BUỘC bên trong `<identification>`

| Thứ tự | Element | Ghi chú |
|---|---|---|
| 1 | `<creator>` (nhiều lần OK) | composer, lyricist, arranger… |
| 2 | `<rights>` | **PHẢI đứng TRƯỚC** `<encoding>` — lỗi cũ đặt sau |
| 3 | `<encoding>` | chứa `<software>`, `<encoding-date>` |
| 4 | `<source>`, `<relation>`… | (nếu có) |
...
**VÍ DỤ SAI → Fatal error line 18 "rights not defined in this scope":**
```xml
<identification>
  <creator type="composer">...</creator>
  <encoding>...</encoding>      ← SAU encoding thì scope đã qua
  <rights>...</rights>          ← LỖI! phải đặt TRƯỚC encoding
</identification>
```

---

#### 11.4.2 Trình tự BẮT BUỘC bên trong `<attributes>`

| Thứ tự | Element | Ghi chú |
|---|---|---|
| 1 | `<divisions>` | Bắt buộc, đầu tiên |
| 2 | `<key>` | fifths + mode |
| 3 | `<time>` | beats + beat-type |
| 4 | `<staves>` | **Chỉ khi >=2 staff** — **PHẢI đứng TRƯỚC** mọi `<clef>`. Đây là lỗi cũ "staves not defined in this scope" |
| 5 | `<part-symbol>` (nếu có) |  |
| 6 | `<instruments>` (nếu có) |  |
| 7 | `<clef>` (1 hoặc nhiều lần) | nếu nhiều staff thì `number="1"` cho staff 1, `number="2"` cho staff 2… |
| 8 | `<staff-details>`, `<transpose>`, `<measure-style>` | (nếu có) |

**VÍ DỤ ĐÚNG (Piano 2 staff):**
```xml
<attributes>
  <divisions>2</divisions>
...
**VÍ DỤ SAI → Fatal error "staves not defined in this scope":**
```xml
<attributes>
  <divisions>2</divisions><key/><time/>
  <clef/>                       ← clef đứng ĐẦU
  <staves>2</staves>            ← LỖI! phải đứng TRƯỚC mọi clef
  <clef number="2"/>
</attributes>
```
...
### 11.5 ✅ Quy tắc nhất quán `<part-list>` ↔ các `<part>` thực tế

**Hard rule (bắt buộc):**  
`COUNT( <score-part> TRONG <part-list> )  PHẢI BẰNG  COUNT( <part> element con của <score-partwise> )`

Và: `mỗi <score-part id="X">  PHẢI có đúng một  <part id="X">` với cùng giá trị id.
...
<!-- Phải có đúng 2 <part> sau: -->
<part id="P1">…56 measures…</part>
<part id="P2">…56 measures…</part>
```

**VÍ DỤ SAI → schema validation fail (lỗi cũ):**
```xml
<part-list>
  <score-part id="P1">…</score-part>
  <score-part id="P2">…</score-part>
  <score-part id="P3">…</score-part>   ← khai báo thêm
  <score-part id="P4">…</score-part>   ← 3 part ảo
  <score-part id="P5">…</score-part>
</part-list>
<part id="P1">…</part>
<part id="P2">…</part>
<!-- thiếu P3, P4, P5 → SAI! -->
...
> 🚨 **Quy tắc an toàn phòng hạn chế token:** Nếu prompt yêu cầu dàn nhạc 5 nhạc cụ nhưng bạn ước tính token/không gian chỉ đủ viết chi tiết 2 part, thì **GIẢM `part-list` xuống còn 2 part** (chọn 2 nhạc cụ quan trọng nhất: giọng hát + piano đệm). Không bao giờ khai báo trong part-list nhiều hơn số part thực tế bạn sẵn sàng viết đầy đủ element XML. Comment kiểu "PARTS 3–5 follow analogous structure... but omitted" BẮT BUỘC phải thay bằng **giảm part-list đi**, không được để comment lừa.

---

### 11.6 📏 Quy tắc tính liên tục Measure (Measure Continuity Rule)

Hard rule:  
**Trong mỗi `<part id="X">`, các `<measure number="N">` phải tạo một chuỗi số nguyên LIÊN TỤC bắt đầu từ 1 và kết thúc ở `MAX_MEASURE` (giá trị lớn nhất xuất hiện trong part giọng hát).**

Tức là: nếu part P1 (Voice) có 56 measure (`number="1"` → `"56"`), thì **mọi** part khác (P2 Piano, P3 Strings…) cũng phải có chính xác 56 `<measure>` với `number="1"` → `"56"` tuần tự.

**Được phép:** Giữ các measures trung gian của part phụ đơn giản (ví dụ: 1 hợp âm whole-note mỗi measure nếu chưa phối khí chi tiết). Nhưng **KHÔNG ĐƯỢC PHÉP:**
- Nhảy số (measure 5 rồi trực tiếp measure 56, bỏ 6–55)
- Thay element `<measure>` bằng comment kiểu `<!-- measures 6–56 continue with harmonic support -->`

**VÍ DỤ ĐÚNG (Piano measure 5 → 6 → ... → 56 liên tục, đệm hợp âm đơn):**
```xml
<part id="P2">
  <measure number="5">
    <note><pitch><step>D</step><octave>4</octave></pitch>
          <duration>8</duration><voice>1</voice><type>whole</type><staff>1</staff></note>
    <backup><duration>8</duration></backup>
    <note><pitch><step>D</step><octave>3</octave></pitch>
          <duration>8</duration><voice>2</voice><type>whole</type><staff>2</staff></note>
  </measure>
  <measure number="6">…measure next hợp âm A…</measure>
  <measure number="7">…Bm…</measure>
  <!-- ... cứ thế liên tục ... -->
  <measure number="55">…chord cuối Outro…</measure>
  <measure number="56">…final barline…</measure>
</part>
```

**VÍ DỤ SAI → schema fail & downstream crash (lỗi cũ):**
```xml
<part id="P2">
  <measure number="5">…chord…</measure>
  <!--  ... measures 6–56 continue with harmonic support  ... -->
  <measure number="56">…</measure>    ← nhảy 50 measure! SAI
</part>
```

> 💡 **Mẹo nhanh nếu thiếu thời gian/không gian:** Với các measure trung gian của part phụ chưa phối khí chi tiết, lặp lại pattern hợp âm root-position whole-note là hoàn toàn chấp nhận được. Pattern này hợp lệ schema, giữ được cấu trúc đúng N measure, và vẫn có thể fine-tune sau. Quan trọng là **không được bỏ measure**.

---

### 11.7 🔒 Cấm "Comment thay Element XML" — Hard Prohibition

Đây là lỗi hệ thống cần chặn tận gốc. Không bao giờ sử dụng các comment dạng sau để "che" việc cấu trúc chưa đầy đủ:

| ❌ ĐƯỢC DÙNG (loại comment này là HỦY BỊ): | Tại sao sai? |
|---|---|
| `<!-- Full expansion omitted here only for response length -->` | Định biện minh cho việc thiếu element. Nếu thiếu element → schema invalid → không xuất. |
| `<!-- measures 6–56 continue with consistent harmonic support -->` | Đang thay **50 `<measure>` element** bằng plain text. Schema không đọc comment → 50 measures bị mất. |
| `<!-- Placeholder continuation to keep schema valid -->` | Comment không giữ schema valid. Chỉ có element XML thực mới giữ. |
| `<!-- PARTS 3–5 (Strings, Brass, Percussion) follow analogous structure... -->` | Nếu đã remove các part này khỏi `<part-list>` thì xóa luôn comment; nếu còn trong part-list thì phải viết `<part>` thực. |

**Comment được phép (chỉ mang tính mô tả):**
- `<!-- INTRO (4 measures, instrumental) -->` (đánh dấu section, không thay element)
- `<!-- Verse 1: Trời thu xanh thẳm gió heo may -->` (gợi ý lời sắp tới, không thay element)
- `<!-- Piano: D major arpeggio feel -->` (chú thích phong cách phối khí, element đã có)
...
----------[ CÁC MỤC KIỂM TRA MỚI — CHỐNG LỖI SCHEMA CỤ THỂ ]----------

✅ [DTD-A] <score-partwise> KHÔNG chứa xmlns:xsi hay xsi:noNamespaceSchemaLocation.
   Chỉ có DOCTYPE DTD duy nhất. Nếu có XSD declaration → XÓA.
✅ [DTD-B] Thứ tự bên trong <identification>: creator* → rights* → encoding → ...
   <rights> PHẢI đứng TRƯỚC <encoding>. Nếu đảo → SỬA.
✅ [DTD-C] Nếu <attributes> có <staves> (>=2 staff):
   divisions → key → time → staves → clef[1] → clef[2] → ...
   <staves> PHẢI đứng TRƯỚC mọi <clef>. Nếu giữa các clef → SỬA.
✅ [STRUCT-A] Số lượng <score-part> trong <part-list> BẰNG chính xác số
   lượng element <part> con của <score-partwise>. Không khai báo part-list
   nhiều hơn số part thực tế có thể viết đủ. Nếu P3,P4,P5 trong part-list
   nhưng không có <part id="P3">,<P4>,<P5> → XÓA P3,P4,P5 khỏi part-list.
✅ [STRUCT-B] Mọi <part id="X"> có cùng số lượng <measure> và cùng chuỗi
   number (P1 có measure 1..56 → P2 cũng phải 1..56 tuần tự). Không được
   nhảy số (measure 5 rồi thẳng measure 56). Nếu thiếu → BỔ SUNG.
✅ [STRUCT-C] Không có XML comment kiểu:
   "omitted for response length", "placeholder continuation",
   "measures X–Y continue", "parts 3–5 follow analogous", etc.
   Mọi comment "xin phép tắt" đang che giấu phần tử thiếu → XÓA comment
   và BỔ SUNG phần tử thật (hoặc GIẢM cấu trúc cho khớp phần tử thật).

---------------------------------------------------------------------------

✅ MusicXML hợp lệ theo schema 4.0 (chạy validator XSD/DTD trước khi xuất).
...
| Xuất MusicXML sai schema. | Validate đầy đủ (mục 13 — **bao gồm 6 mục DTD-A/C, STRUCT-A/C mới**) trước khi xuất. |
| Âm thầm quyết định mà không ghi chú. | Ghi đầy đủ vào `COMPOSITION_NOTES`, kể cả phương án đã loại. |
| **Thêm `xmlns:xsi`/`xsi:noNamespaceSchemaLocation` vào `<score-partwise>` (DTD+XSD xung đột).** | **Chỉ dùng DTD validation qua `<!DOCTYPE score-partwise PUBLIC ...>` một mình như ví dụ 11.1.** |
| **Đặt `<rights>` SAU `<encoding>` trong `<identification>`.** | **Đặt `<rights>` TRƯỚC `<encoding>` theo trình tự DTD (mục 11.4.1).** |
| **Đặt `<staves>` GIỮA hai `<clef>` (hoặc sau clef đầu).** | **Nếu đa-staff: `divisions → key → time → staves → clef[1] → clef[2]` (mục 11.4.2).** |
| **Khai báo P3, P4, P5 trong `<part-list>` nhưng không viết `<part id="P3">,<P4>,<P5>` thực tế.** | **Giảm `part-list` xuống đúng số part thực tế có thể viết đủ. Count bằng nhau; id khớp 1-1 (mục 11.5).** |
| **Nhảy số measure (P2 chỉ có m1–m5 rồi thẳng m56) / dùng comment "measures 6–56 continue" thay element.** | **Mọi part phải có chuỗi measure 1→MAX liên tục (giống P1 Voice). Thiếu → bổ sung, dùng pattern hợp âm whole-note đơn giản nếu chưa phối khí chi tiết (mục 11.6).** |
| **Chèn comment kiểu "Full expansion omitted for response length" / "Placeholder to keep schema valid" / "Parts 3–5 follow analogous".** | **Không dùng comment biện minh cho thiếu element. Nếu cấu trúc chưa đủ → SỬA cấu trúc, không chèn comment "xin phép tắt" (mục 11.7).** |
