(() => {
  ...
  // ========== Grok-SPECIFIC ROW SELECTORS ==========
  const USER_ROW_SELECTORS = [
    '[class*="user-message"]', '[data-testid*="user-message"]',
    '[class*="UserBubble"]', '[class*="user-bubble"]',
    '[class*="chat"][class*="user" i]',
    '[class*="message-user"]'
  ];
  const ASSISTANT_ROW_SELECTORS = [
    'div[class*="message" i]', '[data-testid*="message"]',
    '[class*="assistant-message"]', '[class*="bot-bubble"]',
    '[class*="BotMessage"]', '[class*="ai-message"]',
    '[class*="response"]', '[class*="streaming"]'
  ];
  ...
  const USER_CONTENT_SELECTORS = [
    ...
    '[class*="bubble-content"]', '[class*="user-content"]'
  ];
  ...
  const QUOTA_MARKERS = [
    ...
    /premium only/i, /x premium/i, /subscribe to/i
  ];
  ...
  function detectUiState() {
    ...
    try {
      const stopBtn = first([
        ...
        'button[data-testid*="stop" i]', '[class*="stop"] button', 'button[class*="stop"]'
      ]);
      if (stopBtn) state.loading = true;
      if (!state.loading) {
        const streaming = document.querySelectorAll('[class*="streaming"], [class*="typing-indicator"], [class*="generating"]');
  ...
  function checkPromptAlreadySent(prompt) {
    ...
    try {
      const input = first([
        ...
        'div[contenteditable="true"]', 'textarea', 'textarea[placeholder]'
      ]);
      if (input) {
  ...
  async function waitForAnswer(prompt, alreadySent, timeoutMs) {
    ...
    while (Date.now() < deadline) {
      ...
      if (ui.quotaError) return { ok: false, error: `Grok: hết lượt/quota (banner lỗi). ${ui.errorBannerText ? '→ ' + ui.errorBannerText : ''}` };
      const ctx = getConversationContext(prompt);
      const paired = ctx.answerAfterPrompt;
      if (paired && paired.length > 40) {
        if (paired === last) {
          if (!stableSince) stableSince = Date.now();
          if (!ui.loading && Date.now() - stableSince > 1300) return { ok: true, text: paired };
          if (Date.now() - stableSince > 2800) return { ok: true, text: paired };
      ...
      if (newOnes.length > 0) {
        ...
        if (current === last) {
          if (!stableSince) stableSince = Date.now();
          if (!ui.loading && Date.now() - stableSince > 1300) return { ok: true, text: current };
          if (Date.now() - stableSince > 2800) return { ok: true, text: current };
    ...
    if (uiEnd.quotaError) return { ok: false, error: `Grok: hết lượt/quota khi chờ kết quả. ${uiEnd.errorBannerText ? '→ ' + uiEnd.errorBannerText : ''}` };
    if (uiEnd.loading) return { ok: false, error: 'Grok: hết thời gian chờ (AI vẫn đang xử lý, tab có thể treo).' };
    return { ok: false, error: `Grok: hết thời gian chờ. Hãy refresh tab hoặc kiểm tra giao diện. (Tìm thấy ${ctxEnd.assistantCount} assistant row; pair=${ctxEnd.matchedUserIdx>=0?'Y':'N'})` };
  }

  async function run(prompt, timeoutMs) {
    const uiPre = detectUiState();
    if (uiPre.quotaError) return { ok: false, error: `Grok: Hết lượt/quota (trước khi gửi). ${uiPre.errorBannerText ? '→ ' + uiPre.errorBannerText : ''}` };
  ...
  function peekAllMessages() {
    try {
      const arr = getAssistantAnswers();
      return { ok: true, provider: 'grok', candidates: arr.map((t, i) => ({ index: i, text: t, length: t.length })).filter(c => c.length > 20) };
